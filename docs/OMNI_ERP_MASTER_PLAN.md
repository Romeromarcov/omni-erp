...
| Estado server | TanStack Query v5 | latest | âœ… Implementado (useQuery/useMutation) |
| i18n | react-i18next | latest | âœ… Implementado (es/en, namespace ventas) |
| PWA | vite-plugin-pwa | latest | âœ… Implementado |
| Tests | Vitest + Testing Library | latest | âœ… 29+ tests passing (Fase 0) |

### Infraestructura
| Componente | Estado | Notas |
|---|---|---|
| Docker Compose | âœ… Definido | Backend + PostgreSQL |
| CI/CD | âœ… Implementado | GitHub Actions (.github/workflows/ci.yml) |
...
- [x] **TanStack Query v5** âœ… â†’ todas las pÃ¡ginas hacen fetch manual con `useEffect`. Causa: sin cache, sin re-fetch automÃ¡tico, sin estados de loading/error uniformes
- [ ] **`any` types restantes** (~12 ocurrencias en pÃ¡ginas de detalle) â†’ deben tipificarse con interfaces de respuesta API
- [x] **Tests** âœ… Vitest 29+ tests frontend; pytest 602 tests backend (Fase 0 completa) â†’ crÃ­tico antes de primer cliente en producciÃ³n
