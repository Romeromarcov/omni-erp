...
with override_settings(ALLOWED_HOSTS=["testserver"]):
    ...
    try:
        ...
        if user:
            ...
            response = client.get(
                "/api/finanzas/transacciones-financieras/?id_empresa=44444444-4444-4444-4444-444444444444"
            )
            ...
            if response.status_code == 200:
                ...
                if isinstance(data, list):
                    ...
                    if len(data) > 0:
                        print(
                            f'Primera transacción: ID={data[0].get("id_transaccion")}, Tipo={data[0].get("tipo_transaccion")}'
                        )
                    ...
                    if len(results) > 0:
                        print(
                            f'Primera transacción: ID={results[0].get("id_transaccion")}, Tipo={results[0].get("tipo_transaccion")}'
                        )
                else:
