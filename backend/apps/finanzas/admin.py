...
class MovimientoCajaBancoAdmin(admin.ModelAdmin):
    ...
    def crear_ajuste(self, request, queryset):
        ...
        from apps.finanzas.models import Caja, CuentaBancariaEmpresa, Moneda
...
class CajaFisicaUsuarioAdmin(admin.ModelAdmin):
    ...
    list_filter = (
        ...
        "puede_abrir_sesion",
        "puede_cerrar_sesion",
        "caja_fisica__empresa",
        "caja_fisica__sucursal",
    )
    search_fields = ("usuario__username", "usuario__email", "caja_fisica__nombre")
    readonly_fields = ("id", "fecha_asignacion")

    fieldsets = (
        ("Asignación", {"fields": ("usuario", "caja_fisica")}),
        ("Configuración", {"fields": ("es_predeterminada", "puede_abrir_sesion", "puede_cerrar_sesion")}),
        ("Información", {"fields": ("fecha_asignacion",), "classes": ("collapse",)}),
    )
...
class PagoAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Detalles del Pago", {"fields": ("fecha_pago", "monto", "id_moneda", "tasa", "id_metodo_pago")}),
        (
