...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        usd, created = Moneda.objects.get_or_create(
            codigo_iso="USD", defaults={"nombre": "Dólar", "simbolo": "$", "decimales": 2, "activo": True}
        )
        if created:
        ...
        eur, created = Moneda.objects.get_or_create(
            codigo_iso="EUR", defaults={"nombre": "Euro", "simbolo": "€", "decimales": 2, "activo": True}
        )
        if created:
        ...
        categoria_electronica, created = CategoriaProducto.objects.get_or_create(
            ...
            id_empresa=acme,
            defaults={"descripcion": "Productos electrónicos y tecnológicos", "activo": True},
        )
        if created:
        ...
        categoria_muebles, created = CategoriaProducto.objects.get_or_create(
            ...
            id_empresa=acme,
            defaults={"descripcion": "Muebles y mobiliario", "activo": True},
        )
        if created:
        ...
        unidad_pieza, created = UnidadMedida.objects.get_or_create(
            abreviatura="PZA", defaults={"id_empresa": acme, "nombre": "Pieza", "tipo": "CANTIDAD", "activo": True}
        )
        if created:
