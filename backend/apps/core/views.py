...
def placeholder_usuarios_view(request):
    ...
    from .models import Empresa, Usuarios
    from .serializers import UsuariosSerializer
    ...
    if request.method == "GET":
        ...
        return Response(
            {"detail": "Validation error", "errors": serializer.errors}, status=status.HTTP_400_BAD_REQUEST
        )
...
def placeholder_permisos_view(request):
    ...
    if request.method == "GET":
        ...
        return Response({"id": 3, "message": "Permiso creado exitosamente (mock)"}, status=status.HTTP_201_CREATED)
...
def placeholder_monedas_view(request):
    ...
    if request.method == "GET":
        monedas = [
            {"id": 1, "nombre": "Dólar Estadounidense", "codigo_iso": "USD", "simbolo": "$", "es_activa": True},
            {"id": 2, "nombre": "Euro", "codigo_iso": "EUR", "simbolo": "€", "es_activa": True},
            {"id": 3, "nombre": "Peso Mexicano", "codigo_iso": "MXN", "simbolo": "$", "es_activa": True},
        ]
        return Response(monedas, status=status.HTTP_200_OK)

    elif request.method == "POST":
        return Response({"id": 4, "message": "Moneda creada exitosamente (mock)"}, status=status.HTTP_201_CREATED)
