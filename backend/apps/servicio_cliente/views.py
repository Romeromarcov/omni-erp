...
from .serializers import (
    BaseConocimientoArticuloSerializer,
    CategoriaTicketSerializer,
    FeedbackClienteSerializer,
    InteraccionTicketSerializer,
...
class CategoriaTicketViewSet(TenantFKScopeMixin, viewsets.ModelViewSet):
    ...
    def estadisticas(self, request, pk=None):
        ...
        tickets_abiertos = TicketSoporte.objects.filter(
            id_categoria_ticket=categoria, estado_ticket__in=["ABIERTO", "ASIGNADO", "EN_PROGRESO"]
        ).count()
        tickets_cerrados = TicketSoporte.objects.filter(id_categoria_ticket=categoria, estado_ticket="CERRADO").count()
...
class TicketSoporteViewSet(TenantFKScopeMixin, viewsets.ModelViewSet):
    ...
    def asignar_agente(self, request, pk=None):
        ...
        if not agente_id:
            return Response({"error": "Debe especificar el ID del agente"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def dashboard(self, request):
        ...
        tickets_por_prioridad = queryset.values("prioridad").annotate(count=Count("id_ticket"))

        # Tickets por estado
        tickets_por_estado = queryset.values("estado_ticket").annotate(count=Count("id_ticket"))

        # Tiempo promedio de resolución (últimos 30 días)
        tickets_cerrados_recientes = queryset.filter(
            estado_ticket="CERRADO", fecha_cierre__gte=timezone.now() - timedelta(days=30)
        )
...
class InteraccionTicketViewSet(TenantFKScopeMixin, viewsets.ModelViewSet):
    ...
    def agregar_comentario(self, request):
        ...
        if not ticket_id or not contenido:
            return Response({"error": "ticket_id y contenido son requeridos"}, status=status.HTTP_400_BAD_REQUEST)
...
class BaseConocimientoArticuloViewSet(TenantFKScopeMixin, viewsets.ModelViewSet):
...
class FeedbackClienteViewSet(TenantFKScopeMixin, viewsets.ModelViewSet):
    ...
    def estadisticas_satisfaccion(self, request):
        ...
        distribucion = feedback.values("calificacion").annotate(count=Count("id_feedback")).order_by("calificacion")

        return Response(
            {
                "total_respuestas": total_respuestas,
                "calificacion_promedio": round(calificacion_promedio, 2),
                "distribucion_calificaciones": {str(item["calificacion"]): item["count"] for item in distribucion},
            }
        )
