    ...
    def validate(self, data):
        ...
        if (data.get("es_generico") or data.get("es_publico") or data.get("empresa")) and not getattr(
            ...
            raise serializers.ValidationError(
                "Solo el superusuario puede crear o modificar impuestos genéricos, públicos o de otra empresa."
            )
        ...
        if not es_generico and empresa and codigo:
            ...
            if qs.exists():
                raise serializers.ValidationError(
                    {"codigo": "Ya existe un impuesto con este código para esta empresa."}
                )
        return data
...
class RetencionSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if (data.get("es_generico") or data.get("es_publico") or data.get("empresa")) and not getattr(
            ...
            raise serializers.ValidationError(
                "Solo el superusuario puede crear o modificar retenciones genéricas, públicas o de otra empresa."
            )
        es_generico = data.get("es_generico", getattr(self.instance, "es_generico", False))
        empresa = data.get("empresa", getattr(self.instance, "empresa", None))
        codigo = data.get("codigo", getattr(self.instance, "codigo", None))
        ...
        if not es_generico and empresa and codigo:
            ...
            if qs.exists():
                raise serializers.ValidationError(
                    {"codigo": "Ya existe una retención con este código para esta empresa."}
                )
        return data
...
class ContribucionParafiscalSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if (data.get("es_generico") or data.get("es_publico") or data.get("empresa")) and not getattr(
            ...
            raise serializers.ValidationError(
                "Solo el superusuario puede crear o modificar contribuciones genéricas, públicas o de otra empresa."
            )
        es_generico = data.get("es_generico", getattr(self.instance, "es_generico", False))
        empresa = data.get("empresa", getattr(self.instance, "empresa", None))
        codigo = data.get("codigo", getattr(self.instance, "codigo", None))
        ...
        if not es_generico and empresa and codigo:
            ...
            if qs.exists():
                raise serializers.ValidationError(
                    {"codigo": "Ya existe una contribución con este código para esta empresa."}
                )
        return data
