...
def crear_transaccion_financiera_pago(pago, usuario):
    ...
    try:
        ...
        logger.debug(f"Creando transacción financiera para pago: {pago.id_pago_pedido}, pedido: {pedido.numero_pedido}")
        ...
        if hasattr(pago, 'id_caja_virtual') and pago.id_caja_virtual:
            ...
            if isinstance(pago.id_caja_virtual, Caja) and pago.id_caja_virtual.activa and pago.id_caja_virtual.empresa == empresa:
                ...
                try:
                    ...
                    logger.debug(f"⚠️ Caja virtual seleccionada {getattr(pago, 'id_caja_virtual_id', 'N/A')} no encontrada o inactiva, buscando automáticamente")
        ...
        if hasattr(pago, 'id_cuenta_bancaria') and pago.id_cuenta_bancaria:
            ...
            if isinstance(pago.id_cuenta_bancaria, CuentaBancariaEmpresa) and pago.id_cuenta_bancaria.activo and pago.id_cuenta_bancaria.id_empresa == empresa:
                ...
                try:
                    ...
                    logger.debug(f"✅ Usando cuenta bancaria seleccionada por usuario (por ID): {cuenta_bancaria.nombre_banco}")
                except (CuentaBancariaEmpresa.DoesNotExist, ValueError, AttributeError):
                    logger.debug(f"⚠️ Cuenta bancaria seleccionada {getattr(pago, 'id_cuenta_bancaria_id', 'N/A')} no encontrada o inactiva, buscando automáticamente")
        ...
        if not caja_virtual:
            ...
            if usar_datafono:
                ...
                if hasattr(pago, 'id_datafono') and pago.id_datafono:
                    if isinstance(pago.id_datafono, Datafono) and pago.id_datafono.activo and pago.id_datafono.id_empresa == empresa:
                        ...
                        try:
                            ...
                            logger.debug(f"⚠️ Datafono seleccionado {pago.id_datafono} no encontrado o inactivo, buscando automáticamente")
                ...
                if hasattr(pago, 'banco_destino') and pago.banco_destino:
                    if isinstance(pago.banco_destino, CuentaBancariaEmpresa) and pago.banco_destino.activo and pago.banco_destino.id_empresa == empresa:
                        ...
                        try:
                            ...
                            logger.debug(f"✅ Usando banco destino seleccionado (por ID): {banco_destino.nombre_banco}")
                ...
                if cuenta_bancaria:
                    logger.debug(f"✅ Usando cuenta bancaria automática: {cuenta_bancaria.nombre_banco} - {cuenta_bancaria.numero_cuenta}")
                else:
        ...
        logger.debug(f"Caja/Cuenta/Datafono encontrados: Caja={caja_virtual}, Cuenta={cuenta_bancaria}, Datafono={datafono}")
