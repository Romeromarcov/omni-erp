...
from apps.finanzas.models import Pago, MetodoPago
...
class TestAislamientoVentas:
    ...
    def test_list_solo_devuelve_cotizaciones_propias(
        self, user_a, cotizacion_a, cotizacion_b
    ):
        client = APIClient()
        ...
        assert str(cotizacion_a.id_cotizacion) in ids, \
            "La cotización propia de Empresa A no aparece en el listado."
        assert str(cotizacion_b.id_cotizacion) not in ids, \
            "LEAK: cotización de Empresa B aparece en listado de Empresa A."

    def test_get_cotizacion_ajena_devuelve_404(
        self, user_a, cotizacion_a, cotizacion_b
    ):
        client = APIClient()
        client.force_authenticate(user=user_a)
        ...
        assert resp.status_code == 404, \
            f"LEAK: status {resp.status_code} al acceder a cotización de Empresa B."

    def test_patch_cotizacion_ajena_devuelve_404(
        self, user_a, cotizacion_a, cotizacion_b
    ):
        client = APIClient()
        client.force_authenticate(user=user_a)
        ...
        assert resp.status_code == 404, \
            f"LEAK: status {resp.status_code} al parchear cotización de Empresa B."

        cotizacion_b.refresh_from_db()
        assert cotizacion_b.estado == "BORRADOR", \
...
class TestAislamientoInventario:
    ...
    def test_list_solo_devuelve_categorias_propias(
        self, user_a, categoria_a, categoria_b
    ):
        client = APIClient()
        client.force_authenticate(user=user_a)
        ...
        assert str(categoria_a.id_categoria_producto) in ids, \
            "La categoría propia de Empresa A no aparece en el listado."
        assert str(categoria_b.id_categoria_producto) not in ids, \
            "LEAK: categoría de Empresa B aparece en listado de Empresa A."

    def test_get_categoria_ajena_devuelve_404(
        self, user_a, categoria_a, categoria_b
    ):
        client = APIClient()
        client.force_authenticate(user=user_a)
        ...
        assert resp.status_code == 404, \
            f"LEAK: status {resp.status_code} al acceder a categoría de Empresa B."

    def test_patch_categoria_ajena_devuelve_404(
        self, user_a, categoria_a, categoria_b
    ):
        client = APIClient()
        client.force_authenticate(user=user_a)
        ...
        assert resp.status_code == 404, \
            f"LEAK: status {resp.status_code} al parchear categoría de Empresa B."

        categoria_b.refresh_from_db()
        assert categoria_b.nombre_categoria == "Categoría Beta", \
...
class TestAislamientoFinanzas:
    ...
    def test_list_solo_devuelve_pagos_propios(
        self, user_a, pago_a, pago_b
    ):
        client = APIClient()
        client.force_authenticate(user=user_a)
        ...
        assert str(pago_a.id_pago) in ids, \
            "El pago propio de Empresa A no aparece en el listado."
        assert str(pago_b.id_pago) not in ids, \
            "LEAK: pago de Empresa B aparece en listado de Empresa A."

    def test_get_pago_ajeno_devuelve_404(
        self, user_a, pago_a, pago_b
    ):
        client = APIClient()
        client.force_authenticate(user=user_a)
        ...
        assert resp.status_code == 404, \
            f"LEAK: status {resp.status_code} al acceder a pago de Empresa B."

    def test_patch_pago_ajeno_devuelve_404(
        self, user_a, pago_a, pago_b
    ):
        client = APIClient()
        client.force_authenticate(user=user_a)
        ...
        assert resp.status_code == 404, \
            f"LEAK: status {resp.status_code} al parchear pago de Empresa B."

        pago_b.refresh_from_db()
        assert pago_b.monto == Decimal("200.00"), \
...
class TestAislamientoCompras:
    ...
    def test_list_solo_devuelve_ordenes_propias(
        self, user_a, orden_a, orden_b
    ):
        client = APIClient()
        client.force_authenticate(user=user_a)
        ...
        assert str(orden_a.id_orden_compra) in ids, \
            "La orden propia de Empresa A no aparece en el listado."
        assert str(orden_b.id_orden_compra) not in ids, \
            "LEAK: orden de Empresa B aparece en listado de Empresa A."

    def test_get_orden_ajena_devuelve_404(
        self, user_a, orden_a, orden_b
    ):
        client = APIClient()
        client.force_authenticate(user=user_a)
        ...
        assert resp.status_code == 404, \
            f"LEAK: status {resp.status_code} al acceder a orden de Empresa B."

    def test_patch_orden_ajena_devuelve_404(
        self, user_a, orden_a, orden_b
    ):
        client = APIClient()
        client.force_authenticate(user=user_a)
        ...
        assert resp.status_code == 404, \
            f"LEAK: status {resp.status_code} al parchear orden de Empresa B."

        orden_b.refresh_from_db()
        assert orden_b.estado == "BORRADOR", \
            "CRÍTICO: estado de orden de Empresa B fue modificado desde Empresa A."
...
class TestAislamientoProveedores:
    ...
    def test_list_solo_devuelve_proveedores_propios(
        self, user_a, proveedor_a, proveedor_b
    ):
        client = APIClient()
        client.force_authenticate(user=user_a)
        ...
        assert str(proveedor_a.id_proveedor) in ids, \
            "El proveedor propio de Empresa A no aparece en el listado."
        assert str(proveedor_b.id_proveedor) not in ids, \
            "LEAK: proveedor de Empresa B aparece en listado de Empresa A."

    def test_get_proveedor_ajeno_devuelve_404(
        self, user_a, proveedor_a, proveedor_b
    ):
        client = APIClient()
        client.force_authenticate(user=user_a)
        ...
        assert resp.status_code == 404, \
            f"LEAK: status {resp.status_code} al acceder a proveedor de Empresa B."

    def test_patch_proveedor_ajeno_devuelve_404(
        self, user_a, proveedor_a, proveedor_b
    ):
        client = APIClient()
        client.force_authenticate(user=user_a)
        ...
        assert resp.status_code == 404, \
            f"LEAK: status {resp.status_code} al parchear proveedor de Empresa B."

        proveedor_b.refresh_from_db()
        assert proveedor_b.razon_social == "Proveedor Beta C.A.", \
...
class TestAislamientoGastos:
    ...
    def test_list_solo_devuelve_categorias_propias(
        self, user_a, cat_gasto_a, cat_gasto_b
    ):
        client = APIClient()
        client.force_authenticate(user=user_a)
        ...
        assert str(cat_gasto_a.id_categoria_gasto) in ids, \
            "La categoría de gasto propia de Empresa A no aparece en el listado."
        assert str(cat_gasto_b.id_categoria_gasto) not in ids, \
            "LEAK: categoría de gasto de Empresa B aparece en listado de Empresa A."

    def test_get_categoria_gasto_ajena_devuelve_404(
        self, user_a, cat_gasto_a, cat_gasto_b
    ):
        client = APIClient()
        client.force_authenticate(user=user_a)
        ...
        assert resp.status_code == 404, \
            f"LEAK: status {resp.status_code} al acceder a categoría gasto de Empresa B."

    def test_patch_categoria_gasto_ajena_devuelve_404(
        self, user_a, cat_gasto_a, cat_gasto_b
    ):
        client = APIClient()
        client.force_authenticate(user=user_a)
        ...
        assert resp.status_code == 404, \
            f"LEAK: status {resp.status_code} al parchear categoría gasto de Empresa B."

        cat_gasto_b.refresh_from_db()
        assert cat_gasto_b.nombre_categoria == "Gastos Beta", \
            "CRÍTICO: nombre de categoría de Empresa B fue modificado desde Empresa A."
...
class TestAislamientoNomina:
    ...
    def test_list_solo_devuelve_periodos_propios(
        self, user_a, periodo_a, periodo_b
    ):
        client = APIClient()
        client.force_authenticate(user=user_a)
        ...
        assert str(periodo_a.id_periodo_nomina) in ids, \
            "El período propio de Empresa A no aparece en el listado."
        assert str(periodo_b.id_periodo_nomina) not in ids, \
            "LEAK: período de Empresa B aparece en listado de Empresa A."

    def test_get_periodo_ajeno_devuelve_404(
        self, user_a, periodo_a, periodo_b
    ):
        client = APIClient()
        client.force_authenticate(user=user_a)
        ...
        assert resp.status_code == 404, \
            f"LEAK: status {resp.status_code} al acceder a período de Empresa B."

    def test_patch_periodo_ajeno_devuelve_404(
        self, user_a, periodo_a, periodo_b
    ):
        client = APIClient()
        client.force_authenticate(user=user_a)
        ...
        assert resp.status_code == 404, \
            f"LEAK: status {resp.status_code} al parchear período de Empresa B."

        periodo_b.refresh_from_db()
        assert periodo_b.estado == "ABIERTO", \
