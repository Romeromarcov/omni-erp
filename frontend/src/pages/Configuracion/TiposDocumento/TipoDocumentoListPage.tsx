...
type TipoDocumentoApiResponse = TipoDocumento[] | { results: TipoDocumento[] };

const TipoDocumentoListPage: React.FC = () => {
  const [search, setSearch] = useState('');
  const navigate = useNavigate();

  const { data: tiposDocumento = [], isLoading } = useQuery<TipoDocumentoApiResponse, Error, TipoDocumento[]>({
    queryKey: ['/configuracion_motor/tipos-documento/'],
    queryFn: () => get<TipoDocumentoApiResponse>('/configuracion_motor/tipos-documento/'),
    select: toList,
  });
