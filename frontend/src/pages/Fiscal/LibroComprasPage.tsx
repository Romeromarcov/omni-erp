  ...
  return (
    <PageLayout maxWidth={1100}>
      <div style={{ marginBottom: 20 }}>
        <h2 style={{ margin: '0 0 4px', fontSize: 22, fontWeight: 700 }}>Libro de Compras — SENIAT</h2>
        <p style={{ color: '#6c757d', fontSize: 14, margin: 0 }}>
          Consulta y exporta el libro de compras en formato TXT SENIAT.
        </p>
      </div>
      ...
      {isError && (
        <div style={{ background: '#ffebee', border: '1px solid #f44336', borderRadius: 8, padding: '12px 16px', color: '#c62828' }}>
          ⚠️ {(error as Error)?.message ?? 'Error al cargar el libro de compras.'}
        </div>
      )}
