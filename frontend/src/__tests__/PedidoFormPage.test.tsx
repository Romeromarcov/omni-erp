/**
 * FE-CRIT-1 characterization tests for PedidoFormPage.
...
// Pedido submit goes directly through services/api post/patch.
const postMock = vi.fn().mockResolvedValue({ id_pedido: 'ped-new', numero_pedido: 'PED-1' });
const patchMock = vi.fn().mockResolvedValue({ id_pedido: 'ped-1', numero_pedido: 'PED-1' });
const getMock = vi.fn();
...
function setGetRouting(editRecord?: unknown) {
  getMock.mockImplementation((url: string) => {
    if (editRecord && url.includes('/ventas/pedidos/')) return Promise.resolve(editRecord);
...
describe('PedidoFormPage (characterization)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    paramsMock = {};
    localStorage.clear();
  ...
  it('does not call the create mutation on an empty/invalid submit', async () => {
    renderForm();
    const submit = await screen.findByRole('button', { name: /guardar pedido/i });
    expect(submit).toBeDisabled();
    expect(postMock).not.toHaveBeenCalled();
  });

  it('loads values when editing an existing pedido', async () => {
    paramsMock = { id_pedido: 'ped-1' };
    setGetRouting(EXISTING_PEDIDO);
    renderForm();
    await waitFor(() => {
      expect(screen.getByDisplayValue('Observacion pedido')).toBeInTheDocument();
    });
  });
