/**
 * FE-CRIT-1 characterization tests for FacturaFiscalFormPage.
...
// FacturaFiscal submit goes through FacturaFiscalService (create→post, update→patch via services/api).
const postMock = vi.fn().mockResolvedValue({ id_factura: 'fac-new', numero_factura: 'FAC-1' });
const patchMock = vi.fn().mockResolvedValue({ id_factura: 'fac-1', numero_factura: 'FAC-1' });
const getMock = vi.fn();
...
const EXISTING_FACTURA = {
  id_factura: 'fac-1',
  numero_factura: 'FAC-0001',
  fecha_emision: '2026-01-10',
  id_empresa: 'emp-1',
  id_cliente: { id_cliente: 'cli-1' },
  observaciones: 'Observacion factura',
  detalles: [
    { id_producto: 'p1', cantidad: 2, precio_unitario: 10, descuento_porcentaje: 0 },
  ],
};

function setGetRouting(editRecord?: unknown) {
  getMock.mockImplementation((url: string) => {
    if (editRecord && url.includes('/ventas/facturas-fiscales/')) return Promise.resolve(editRecord);
...
function renderForm() {
  ...
  return render(
    <QueryClientProvider client={queryClient}>
      <MemoryRouter>
        <FacturaFiscalFormPage />
      </MemoryRouter>
    </QueryClientProvider>
  );
}

describe('FacturaFiscalFormPage (characterization)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    paramsMock = {};
    localStorage.clear();
  ...
  it('does not call the create mutation on an empty/invalid submit', async () => {
    renderForm();
    const submit = await screen.findByRole('button', { name: /guardar factura/i });
    expect(submit).toBeDisabled();
    expect(postMock).not.toHaveBeenCalled();
  });

  it('loads values when editing an existing factura fiscal', async () => {
    paramsMock = { id_factura: 'fac-1' };
    setGetRouting(EXISTING_FACTURA);
    renderForm();
    await waitFor(() => {
      expect(screen.getByDisplayValue('Observacion factura')).toBeInTheDocument();
    });
  });
